...
export default function CustomerManager() {
  ...
  const openEdit = (c: any) => { setForm(c); setEditId(c.id); setShowModal(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ msg: editId ? 'Customer updated' : 'Customer added', type: 'success' });
      setShowModal(false);
      loadData();
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.del(`/api/customers/${deleteId}`);
  ...
  return (
    <div>
      ...
      {deleteId && <ConfirmModal title="Delete Customer" message="Are you sure?" danger onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />}
      
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h2 className="page-title">Customers</h2>
          <p className="page-subtitle">Manage customer CRM and loyalty points</p>
        </div>
      ...
      <div className="card">
        <div className="table-wrapper">
          <table className="data-table">
            <thead>
              <tr><th>Name</th><th>Contact</th><th>Address</th><th>Loyalty Points</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {customers.map(c => (
                <tr key={c.id}>
                  <td style={{ fontWeight: 600 }}>{c.name}</td>
                  <td style={{ fontSize: 13 }}>{c.phone}<br/><span style={{ color: 'var(--color-text-secondary)' }}>{c.email}</span></td>
                  <td>{c.address || '—'}</td>
                  <td>
                    <span className="badge badge-success" style={{ fontSize: 14 }}>★ {c.loyalty_points}</span>
                  </td>
                  <td>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: 12 }} onClick={() => openEdit(c)}>Edit</button>
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header"><h3>{editId ? 'Edit Customer' : 'Add Customer'}</h3></div>
